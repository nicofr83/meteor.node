...
let DB = (() => {
    ...
    var DB = class {
        ...
        static {
            ...
            DB = _classThis = _classDescriptor.value;
        ...
        async beginTransaction(appDbConn = undefined) {
            var myConn;
        ...
        async commitTransaction(myConn) {
            if (myConn == undefined) {
                throw new Error('No connection to database');
            }
            await this.executeSQL(myConn, 'COMMIT');
        }
        async rollbackTransaction(myConn) {
            if (myConn == undefined) {
                throw new Error('No connection to database');
            }
            await this.executeSQL(myConn, 'ROLLBACK');
        }
        async query(appDbConn, sql, values = []) {
        ...
        async execute(myConn, sql, values = []) {
            const rows = await this.query(myConn, sql, values);
            return rows[0];
        }
    };
    return DB = _classThis;
})();
exports.DB = DB;
