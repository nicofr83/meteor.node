...
const jsonLoader_interface_1 = require("../../dataLoader/json/jsonLoader_interface");
function checkJson(dataStations, filename) {
    var meteor = '';
    var ret;
    try {
...
function _checkMeteor(meteor, dataOneStation) {
...
function _checkInfo(meteor, dataOneStation) {
...
function _checkJsonOneMesure(meteor, dataJson) {
    const valeursToAdd = [];
    const extremes_to_add = [];
    var new_val;
    var new_val_xtreme;
    const stop_dat_list = [];
    var tmpDate;
    ...
    for (const key in dataMesure) {
        if (jsonLoader_interface_1.dataJsonKeys.indexOf(key) === -1) {
